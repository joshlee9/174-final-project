...
def main():
    ...
    with open(output_file, 'w') as f:
        for panorama_name in os.listdir(stitched_dir):
            ...
            snr_value = calculate_snr(reference_image, stitched_image)
            f.write(f"{panorama_name}: SNR = {snr_value:.2f} dB\n")
            print(f"{panorama_name}: SNR = {snr_value:.2f} dB")
